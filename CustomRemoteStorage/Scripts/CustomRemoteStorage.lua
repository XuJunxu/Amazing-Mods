...
function CustomRomoteStorage:OnEnter()
	...
	Event:RegisterEvent(g_emEvent.BuildingFinished, function(evt, thing, objs)
		self:AddBtn2Build2(evt, thing, objs); 
	end, "CustomRomoteStorage_AddBtn2Build2");	
	...
	print("CustomRomoteStorage V2.02");
end
...
function CustomRomoteStorage:AddBtn2Build(evt, thing, objs)  --向添加按键
	...
	if thing:IsSleeveSpace() then
		thing:RemoveBtnData(GLS("自定义"));
		if thing.BuildingState == CS.XiaWorld.g_emBuildingState.Working then
			thing:AddBtnData(GLS("自定义"), "res/Sprs/ui/icon_huafu01", "GameMain:GetMod('CustomRomoteStorage'):OpenConfigWindow();", "");
		end
	end
end

function CustomRomoteStorage:AddBtn2Build2(evt, thing, objs)  --向添加按键
	if thing == nil or not self.mod_enable then
		return;
	end
	if thing:IsSleeveSpace() then
		thing:RemoveBtnData(GLS("自定义"));
...
function CustomRomoteStorage:RightClickItem(evt, thing, objs)  --乾坤界添加右键点击回调
	...
	if window ~= nil and window:GetType() == typeof(CS.Wnd_RemoteStorage) and window.contentPane ~= nil and window.contentPane.m_n5 ~= nil then
		window.contentPane.m_n5.onRightClickItem:Add(CustomRomoteStorage.AddCommonItem);
		Event:UnRegisterEvent(g_emEvent.WindowEvent, "CustomRomoteStorage_RightClickItem");
	end
end
...
function CustomRomoteStorage.AddCommonItem(context)
...
function CustomRomoteStorage:GetAllItemData()  --获取所有可以自行设置的物品，会过滤没有Def的物品
	...
	for _, name in pairs(ThingMgr.s_AllItemNames) do
		if all_item_data[name] == nil then
			...
			if def ~= nil and def.Item ~= nil and def.Item.Lable ~= nil and (self.banLable[def.Item.Lable] == nil or self.additionItems[name] ~= nil) and n == nil then
				local item_data = nil;
				if self.all_item_data[name] ~= nil then
...
CustomRomoteStorage.banLable = {
	...
	[g_emItemLable.Cape] = GLS("披风"),
	[g_emItemLable.Horse] = GLS("灵契"),
};

CustomRomoteStorage.additionItems = {
	["Item_StrangeMush"] = g_emItemLable.Other,
};
