...
local SetAreaWindow = Windows:CreateWindow("SetAreaWindow");
local StoreCorpseArea = GameMain:GetMod("StoreCorpseArea");

SetAreaWindow.area_type = {
	["Storage"] = "仓库",
	["Plant"] = "农田",
	["DisciplePlant"] = "药田"
};

function SetAreaWindow:OnInit()
	self.window.contentPane = UIPackage.CreateObject("StoreCorpseArea", "SetAreaWindow");
	...
	self.btn1.onClick:Add(function(context)
		StoreCorpseArea.auto_search = self.check1.selected;
		StoreCorpseArea.auto_slaughter = self.check2.selected;
		local area_list = {};
		...
		StoreCorpseArea.area_list = area_list;
		self:Hide();
	end);
...
function SetAreaWindow:OnShowUpdate()
	self.window:BringToFront();
		self.check1.selected = StoreCorpseArea.auto_search;
		self.check2.selected = StoreCorpseArea.auto_slaughter;
	self.list:RemoveChildrenToPool();
	if StoreCorpseArea.area_list == nil then
		return;
	end
	for _, area_set in pairs(StoreCorpseArea.area_list) do
		local area = AreaMgr:FindAreaByID(area_set.id);
		self:AddSelectedArea(area, area_set);
	end
end

function SetAreaWindow:AddSelectedArea(area, set_data)
	if area == nil then
		return;
	end
...
function SetAreaWindow:OnUpdate(dt)
	if StoreCorpseArea.shift and CS.UnityEngine.Input.GetKey(CS.UnityEngine.KeyCode.Mouse0) then
