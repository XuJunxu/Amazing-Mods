local StoreCorpseArea = GameMain:NewMod("StoreCorpseArea");--先注册一个新的MOD模块

StoreCorpseArea.shift = false;
StoreCorpseArea.savedata = {};

function StoreCorpseArea:OnInit()
	...
	print("StoreCorpseArea Init");
end

function StoreCorpseArea:OnEnter()
	SetAreaWindow = GameMain:GetMod("Windows"):GetWindow("SetAreaWindow");
	SetAreaWindow:Init();
...
function StoreCorpseArea:AutoProcessNpc(evt, npc, objs)
...
function StoreCorpseArea:GetAvailableGrid(atype)
	if atype == nil then
		return nil;
	end
	local list = {};
...
function StoreCorpseArea:OnSetHotKey()
	local tbHotKey = { 
		{ID = "SelectArea" , Name = "选择区域" , Type = "Mod", InitialKey1 = "LeftShift"}, 
		{ID = "ConfigArea" , Name = "设置区域" , Type = "Mod", InitialKey1 = "RightControl+L"}};
	return tbHotKey;
end

function StoreCorpseArea:OnHotKey(ID, state)
	if ID == "SelectArea" then
		if state == "down" then 
			StoreCorpseArea.shift = true;
		elseif state == "up" then 
			StoreCorpseArea.shift = false;
		end   
    end
	if ID == "ConfigArea" and state == "down" then
		SetAreaWindow:Open();
	end
end

function StoreCorpseArea:OnStep(dt)--请谨慎处理step的逻辑，可能会影响游戏效率
...
function StoreCorpseArea:OnLeave()
	if SetAreaWindow.window.isShowing then
		SetAreaWindow:Hide();
	end
	print("StoreCorpseArea Leave");
end

function StoreCorpseArea:OnSave()--系统会将返回的table存档 table应该是纯粹的KV
	local savedata = {
...
function StoreCorpseArea:OnLoad(tbLoad)--读档时会将存档的table回调到这里
