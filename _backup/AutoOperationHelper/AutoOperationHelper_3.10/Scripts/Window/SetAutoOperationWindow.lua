...
function SetAutoOperationWindow:OnInit()
	...
	self.tips.title = "说明";
	self.tips.tooltips = "按住左Shift键，鼠标左键点击仓库/农田/药田中的任意位置将该区域添加到设置面板，设置该区域作为弥留/尸体/昏迷小人的存放区以及使用优先级，当敌人变成弥留/尸体/昏迷时就会被自动搬运到对应区域。\n\n按住左Shift键，鼠标右键点击设置面板上的区域名，可以将该区域删除。\n\n如果出现不会自动搬运，可以打开设置面板后直接点确定。";
...
function SetAutoOperationWindow:AddSelectedArea(area, set_data)
	if area == nil or self.area_temp[area.ID] ~= nil then
		return;
	end
