...
function MultiOperationHelper:MultiEatItem(item)  --选择食用的npc
	CS.Wnd_SelectNpc.Instance:Select(
		WorldLua:GetSelectNpcCallback(function(rs)
			...
			self.NumberSettingWindow:Open("食用(吞噬)", nil, nil, nil, input_item, function(inputdata)
				...
				for _, id in pairs(rs) do
					...
					if npc ~= nil then
						if npc.GongKind == CS.XiaWorld.g_emGongKind.Body and item.Rate > npc:GetProperty("BodyPractice_EatItemMaxRate") then
							world:ShowMsgBox(string.format("[color=#0000FF]%s[/color]暂时无法吞噬品阶大于%0.2f的物品。", npc:GetName(), npc:GetProperty("BodyPractice_EatItemMaxRate")));
						elseif not item:EatAble() and npc.GongKind ~= CS.XiaWorld.g_emGongKind.Body then
							world:ShowMsgBox(string.format("[color=#0000FF]%s[/color]非体修，无法吞噬该物品。", npc:GetName()));
						else
							for i=1, num do
	...
	if magic_def.Name == "AbsorbGong_5" or magic_def.Name == "AbsorbGong_6" then
		CS.Wnd_SelectNpc.Instance:Select(
			WorldLua:GetSelectNpcCallback(function(rs)
				...
				if #names1 > 0 then
					world:ShowMsgBox(string.format("[color=#0000FF]%s[/color]正在被施展神通，暂时无法成为神通对象，请取消神通或稍候再试。", table.concat(names1, "、")));
				end
				if #names2 > 0 then
					world:ShowMsgBox(string.format("[color=#0000FF]%s[/color]与[color=#0000FF]%s[/color]为不同种族，无法被他化或寄生。", table.concat(names2, "、"), npc:GetName()));
				end	
	...
	elseif magic_def.Name == "Healing_Damage" or magic_def.Name == "BuildBody" then
		CS.Wnd_SelectNpc.Instance:Select(
			WorldLua:GetSelectNpcCallback(function(rs)
				...
				self.NumberSettingWindow:Open("施展神通", nil, nil, nil, input_item, function(inputdata)
					if inputdata[1].data == 0 then
						return;
					end
					local npc_ids = {};
					...
					if #names > 0 then
						world:ShowMsgBox(string.format("[color=#0000FF]%s[/color]正在被施展神通，暂时无法成为神通对象，请取消神通或稍候再试。", table.concat(names, "、")));
					end
					self:Magic2Cast(magic_def, npc, npc_ids, index);
				end);
			end), 
		CS.XiaWorld.g_emNpcRank.Normal, 1, 99, nil, nil, magic_def.DisplayName);
	else
...
MultiOperationHelper.operationDef = {
	...
	multiEatItem = {bg = 0, grayed = false, dspName = "多人食用(吞噬)", icon = "res/Sprs/ui/icon_shiyong01", data = "MultiEatItem", tooltips = "选择单个或多个小人，食用同品阶同名称的物品，并可设置连续食用多次（包括体修的吞噬等）。"};
	multiDevourItem = {bg = 0, grayed = false, dspName = "批量吞噬", icon = "res/Sprs/ui/icon_shiyong01", data = "MultiEatItem", tooltips = "选择单个或多个体修小人，吞噬同品阶同名称的物品，并可设置连续吞噬多次。"};
