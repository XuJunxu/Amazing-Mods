...
local LanStr = {
	...
	["取消剩余的所有炼宝任务，正在炼制的法宝不会被取消。"] = "Cancel all remaining tasks of artifact crafting.",
	["取消剩余的所有吞噬命令。"] = "Cancel all remaining tasks of Devouring items.",
...
function MultiOperationHelper:OnEnter()
	...
	print("MultiOperationHelper V4.02");
end
...
function MultiOperationHelper:AddBtn2CancelDevour(npc)
	npc:RemoveBtnData(GLS("取消批量吞噬"));
	if npc.IsPlayerThing and npc.Rank == CS.XiaWorld.g_emNpcRank.Disciple and npc.GongKind == CS.XiaWorld.g_emGongKind.Body then
		local cmds = npc:CheckCommand("EatItem");
		if cmds ~= nil and cmds.Count > 0 then
			local opt = self.operationDef.cancelMultiDevour;
			npc:AddBtnData(opt.dspName, opt.icon, self:GetBtnLua(opt.data, opt.dspName), opt.tooltips, nil);
		end
	end
end

function MultiOperationHelper:CancelMultiDevour(npc)
	if npc ~= nil then
		npc:RemoveCommand("EatItem");
	end
end
...
function MultiOperationHelper:AddStorageMenu(evt, thing, objs)  --乾坤界添加右键菜单
	...
	if window ~= nil and window:GetType() == typeof(CS.Wnd_RemoteStorage) and window.contentPane ~= nil and window.contentPane.m_n5 ~= nil then
		window.contentPane.m_n5.onRightClickItem:Add(self.ShowStorageMenu);
		Event:UnRegisterEvent(g_emEvent.WindowEvent, "MultiOperationHelper_AddStorageMenu");
	end
end

function MultiOperationHelper.ShowStorageMenu(context)
...
function MultiOperationHelper:CheckCanUse(it)  --CollectToRemoteStorage
	return (it.InWhoseBag <= 0 and it.InWhoseHand <= 0 and it.AtG and it.Actable and (not it.InDark) and CS.XiaWorld.TongLingMgr.Instance:GetTongLingData(it.ID) == nil and
			it.LingPower <= 0 and it.FSItemState <= 0 and it.HelianValue == nil and it.pAnimal == nil and it.NoStackFlag == 0);    --ItemThing.CanStack    Thing.CanStack
end
...
function MultiOperationHelper:CheckItemCount(item, count, condition)  --检查物品可操作的数量
	...
	if item_all ~= nil and item_all.Count > 0 then
		for _, it in pairs(item_all) do
			if it.Rate == item.Rate and (condition == nil or condition(it)) and (it.TagData:CheckTagString("_Remote") or self:CheckCanUse(it)) then
...
function MultiOperationHelper:GetActableItems(item, count, condition)  --获取可操作的物品
	...
	if item_all ~= nil and item_all.Count > 0 then  --先使用乾坤界外的物品
		for _, it in pairs(item_all) do
			if (not it.TagData:CheckTagString("_Remote")) and it.Rate == item.Rate and (condition == nil or condition(it)) and self:CheckCanUse(it) then
...
MultiOperationHelper.operationDef = {
	...
	cancelRefiningFabao = {bg = 0, grayed = false, dspName = GLS("取消批量炼宝"), icon = "res/Sprs/ui/icon_lianbao01", data = "CancelRefiningFabao", tooltips = GLS("取消剩余的所有炼宝任务，正在炼制的法宝不会被取消。")};
	cancelMultiDevour = {bg = 0, grayed = false, dspName = GLS("取消批量吞噬"), icon = "res/Sprs/ui/icon_shiyong01", data = "CancelMultiDevour", tooltips = GLS("取消剩余的所有吞噬命令。")};
};
