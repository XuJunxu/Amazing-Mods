...
MagicBtnWindow.setMagic_tips = {
	["SoulCrystalYouPowerUp"] = "按住左Shift键选定物品。若物品的堆叠数=1，则可以设置幽淬的次数；若物品的堆叠数>1，则可以设置幽淬的个数。幽淬次数范围0--12，幽淬个数范围0--99。",
	["FengshuiItemOpen"] = "按住左Shift键选定物品。",
--	["FengshuiItemCreate"] = "按住左Shift键选定物品。若物品的堆叠数>1，则可以设置凝练的个数。凝练个数范围0--99",
};
...
function MagicBtnWindow:ClickBtn(context)
	...
	elseif magic_def.Name == "Prophesy_MapStory" or magic_def.Name == "LingCrystalMake" or magic_def.Name == "LingStoneMake" then
		...
		input_item, 220, true, nil, nil, magic_def.Name);
	...
	elseif magic_def.Name == "SoulCrystalYouPowerUp" or magic_def.Name == "FengshuiItemCreate" or magic_def.Name == "FengshuiItemOpen" then
		...
		nil, 300, false, nil, g_emThingType.Item, magic_def.Name);
	...
	elseif magic_def.Name == "SeachSoul" or magic_def.Name == "PlantGrowUp" or magic_def.Name == "PlantGrowUp_Gong1" or magic_def.Name == "PlantGrowUp_Gong9" then
		...
		nil, 300, false, nil, thing_type, magic_def.Name);
		SetMagicCastWindow.window:LeftTop();	
	end
end
...
function MagicBtnWindow:AddMagicCommand(magic_def, npc, targets, count)  
	if magic_def.CMD ~= nil then
		local CommandTypeDef = CS.XiaWorld.CommandMgr.Instance:GetDef(magic_def.CMD);
	...
	for i=0, count-1 do  --参考NpcMagicBnt.GetBntData()
		...
		elseif magic_def.SelectType ~= CS.XiaWorld.g_emIndividualCommandType.None then
			...
			elseif magic_def.SelectType == CS.XiaWorld.g_emIndividualCommandType.DieStay or magic_def.SelectType == CS.XiaWorld.g_emIndividualCommandType.Plant then
				self:ThingSetIcon(thing);
				local command = npc:AddCommand(magic_def.CMD, thing);
				command.WorkParam3 = magic_def.Name;
				command.EventOnFinished = function() 
