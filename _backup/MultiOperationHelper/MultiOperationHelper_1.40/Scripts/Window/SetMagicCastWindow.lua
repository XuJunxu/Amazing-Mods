...
function SetMagicCastWindow:OnUpdate(dt)
	...
	if thing == self.last_thing or self._stype == nil then
		return;
	end
...
function SetMagicCastWindow:MultiSelectThing(thing)
	if thing ~= nil and thing.ThingType == self._stype and MultiOperationHelper.shift and (MagicBtnWindow:CheckThing(self._magic, thing) or thing.ThingType == g_emThingType.Building) then
